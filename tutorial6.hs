...
isBloodOrange (Orange b c) = b `elem` ["Tarocco", "Moro", "Sanguinello"]
isBloodOrange (Apple b c) = False

-- 2.
bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments xs = length(filter isBloodOrange xs)

-- 3.
worms :: [Fruit] -> Int
worms xs = length(filter worm xs)
  where 
    worm:: Fruit->Bool
    worm (Orange _ _) = False
    worm (Apple _ b) = b == True 
...
tautology p 
  |satisfiable p = and [ eval x p | x <- (envs (atoms p))]
  |otherwise = False
...
equivalent a b = and [ y==z | (y,z) <- zip[ eval x a | x <- (envs (nub (atoms a ++ atoms b)))] [ eval x b | x <- (envs (nub (atoms a ++ atoms b)))]]
...
-- 9.
wff5 = (( V P :|: V Q) :&: ((Not (V P))))

wff6 = (( V P :->: V Q) :|: (V P :<->: V Q)) 

equivalent' :: Eq a => Wff a -> Wff a -> Bool
equivalent' a b = undefined
...
impElim x = x -- (V a), T, F To produce conjunctive nor
